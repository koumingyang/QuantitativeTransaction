...
double genBmaWeights (mat Y, mat X, double propsigma_beta)
{
    ...
    while (s < N)                                           //Iterate until we have N samples
    {
        ...
        //printf("genBmaWeights p3.2.2 %d\n", s);fflush(stdout);
        
        //rho should be in (-1,1)
        while (new_b(1) < -1 || new_b(1) > 1)
            new_b = mvnrnd( prop_mu , prop_Sigma );
        sigma2_v_state = state_Sigma(1,1); 
        //printf("genBmaWeights p3.2.3 %d\n", s);fflush(stdout);
        pratio = exp(-0.5 * pow((x0 - new_b(0) / (1.0 - new_b(1))), 2.0) * (1.0 - new_b(1) * new_b(1)) / sigma2_v_state) /
                 exp(-0.5 * pow((x0 - state_b(0) / (1.0 - state_b(1))), 2.0) * (1.0 - state_b(1) * state_b(1)) / sigma2_v_state);
        alpha = min(1.0, pratio);                           //Calculate the acceptance ratio
        tmp.randu();
        u = tmp(0, 0);                               //Draw a uniform deviate from [ 0 1 ]
        //printf("genBmaWeights p3.2.4 %d %d %d\n", s, new_b.n_rows, new_b.n_cols);fflush(stdout);
        if (u < alpha)                                      //Do we accept this proposal?
            state_b = new_b.col(0);                         //proposal becomes new value for b
        
        //printf("genBmaWeights p3.3 %d\n", s);fflush(stdout);
